...
from .parsers import search_post_parser, user_add_post_parser
...
class ParamConfig(Resource):
    ...
    def post(self):
        try:
            ...
            # 获取请求参数
            if Role == "user":
                pass_tag, args = user_add_post_parser.parse_args() # strict=True, 不允许有额外参数
                if not pass_tag: # 参数验证不通过(名称、类型、数据结构)
                    return {'status': 0, 'msg': args}
            else:
                pass
            # pass_tag, args = cofig_post_parser.parse_args() # strict=True, 不允许有额外参数
            if not pass_tag: # 参数验证不通过(名称、类型、数据结构)
                return {'msg': args}
